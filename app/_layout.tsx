...
console.log('[IRIS] Root layout module evaluating');
...
function SplashHider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    console.log('[IRIS] SplashHider isLoading:', isLoading, 'isDataReady:', isDataReady, 'appReady:', appReady);
    if (isDataReady && !appReady) {
      console.log('[IRIS] Data ready — setting appReady and hiding splash');
      setAppReady(true);
  ...
  useEffect(() => {
    const timeout = setTimeout(() => {
      if (!appReady) {
        console.log('[IRIS] Force hiding splash after timeout');
        setAppReady(true);
  ...
  if (!appReady) {
    console.log('[IRIS] SplashHider: not ready, rendering blank view');
    return <View style={[splashStyles.root, { backgroundColor: bgColor }]} />;
  }

  console.log('[IRIS] SplashHider: ready, rendering children');
...
function DeepLinkHandler({ children }: { children: React.ReactNode }) {
  const onCodeReceived = useCallback((code: string) => {
    console.log("[DeepLink] Referral code received in layout:", code);
  }, []);
...
export default function RootLayout() {
  console.log('[IRIS] RootLayout rendering');
  return (
