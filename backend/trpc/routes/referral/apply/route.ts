...
export default publicProcedure
  ...
  .mutation(({ input }) => {
    ...
    if (!referralStore.isValidCodeFormat(code)) {
      console.log("[Referral API] Apply failed - invalid code format:", code);
    ...
    if (entry && entry.userId === newUserId) {
      console.log("[Referral API] Apply failed - self referral:", newUserId);
      return { success: false as const, error: "self_referral" as const };
    }

    const alreadyReferred = referralStore.getReferredUser(newUserId);
    if (alreadyReferred) {
      console.log("[Referral API] Apply failed - already referred:", newUserId);
    ...
    if (entry) {
      ...
      if (referralsThisMonth >= 10) {
        console.log("[Referral API] Apply failed - monthly limit reached for:", entry.userId);
        return { success: false as const, error: "monthly_limit" as const };
      }

      referralStore.applyReferral(entry.userId, newUserId, code);
      console.log("[Referral API] Referral applied:", code, "new user:", newUserId, "referrer:", entry.userId);
    ...
    console.log("[Referral API] Code format valid but referrer not in store (cold start). Accepting code:", code);
